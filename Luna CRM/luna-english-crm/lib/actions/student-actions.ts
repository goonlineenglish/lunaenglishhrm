...
export async function getStudents(
  ...
  // Server-side search across student fields and joined lead fields
  if (search) {
    const term = `%${search}%`;
    query = query.or(
...
export async function bulkChangeStudentStatus(
  ...
  if ((newStatus === "paused" || newStatus === "dropped") && !reason?.trim()) {
    return {
      succeeded: [],
      failed: studentIds.map((id) => ({ id, error: "Cần có lý do" })),
    };
  }
  ...
  for (const id of studentIds) {
    if (!UUID_RE.test(id)) {
      result.failed.push({ id, error: "ID không hợp lệ" });
    } else {
      validIds.push(id);
    }
  }
  ...
  try {
    ...
    // Batch fetch: get all current statuses in 1 query
    ...
    // Batch update: 1 query for all eligible students
